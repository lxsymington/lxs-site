...
other media queries than just the ones for detecting screen size. Among them are the resolution media queries that even 
work in Internet Explorer back to version 9, but has not been implemented in Safari. These enable one to scope styles 
to: an exact resolution, a minimum resolution and above and a maximum resolution and below. For Safari there are the 
non-standard webkit device pixel ratio queries which are more or less equivalent to the resolution queries. I have 
created a `CSS` custom property (variable) called `--dpr` set to 1, which is then updated to the relevant values by
making use of the minimum resolution and device pixel ratio media queries. This variable can then be used via the 
`var()` function. By dividing a single `CSS` pixel by the device pixel ratio lets us specify a single device pixel 
instead.
...
The above result is fairly smooth for the reasons discussed earlier, but let's rotate the grid slightly and see how that 
compares!
...
```css showcase caption=30_degree_Gradient_Lines
background-image:
  repeating-linear-gradient(
    30deg,
    transparent 0 12px,
...
```css showcase caption=60_degree_Gradient_Lines
background-image:
  repeating-linear-gradient(
    60deg,
    transparent 0 12px,
...
```css showcase caption=Graded_60_degree_Gradient_Lines
background-image:
  repeating-linear-gradient(
    60deg,
    transparent 0 11px,
