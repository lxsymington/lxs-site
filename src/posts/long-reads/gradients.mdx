...
**subtracts** light. So going back to `sRGB`, if we now expand the acronym to the full name `standard Red Green Blue`
...
Having discussed colour representation we should probably get back to the `linear-gradient` example.

<sup>1</sup>At the time of writing Lab and LCH colorspaces have been defined in the CSS color level 4 spec but are yet to be
