...
intelligent use of light may subtly direct visual tracking of a page.
...
gradients _can_, but do not necessarily need to, cause problems in terms of accessibility. Much the same as with text over
images, text over gradients can often cause readability issues. These readability issues often arise from a lack of
...
are pre-defined which is quite limiting &mdash; the web would be rather homogeneous if we could only use 140 colours! So
what did that `sRGB` `colorspace` bit mean? Well there are lots of different ways of thinking about colour, but one that
...
in contrast to paint or ink based colour systems which are called "subtractive" &mdash; adding dye or pigment filters
out light. So going back to `sRGB`, if we now expand the acronym to the full name `standard Red Green Blue` `colorspace`
the reason for choosing red, green and blue light in the earlier thought experiment may become apparent. As a side note
the reason for red, green and blue being used as the _primary colours_ is purportedly related to the types of cone cells
there are in the human retina, and the colour of the light they are most sensitive to. The last thing to address is the
`standard` part of `sRGB`, it is important not to skip over this as it reflects that `sRGB` is device-independent, so
regardless of a devices capabilities provided that it is `sRGB` capable it will display the same colour. How does all
this relate to gradients? We now know that we can represent colours as a list of amounts of red, blue and green light,
amounts otherwise known as _'channels'_. For example `red: 50%, green: 60%, blue: 70%`.
...
be rounded to the nearest incerement. The good news is that we have finally established what colour the first row of
pixels in our gradient should be painted: `rgb(1, 1, 1)` or `#010101` and not only that but also a methodology for doing
the same for subsequent rows. For example, the second row, is offset 1px more than the first but in all other dimensions
is the same. So the midpoint of the second row will also be offset 1px from the first row's midpoint, 1&frac12; pixels
...
Now at a glance that can seem quite daunting, so lets try to break it down a bit. Working from left to right, we have
...
That was an implementation that was manufactured for convenience though. So lets have a look at using the formula with
some arbitrary colours, `mediumspringgreen` (`#00fa9a`) and `mediumpurple` (`#9370db`) for example and find the midpoint
colour.  To start with lets convert the hexadecimal values to decimal so that they're a bit more readable. Just as
...
$$
{\displaystyle RedChannelMidpoint=0+(50-0){\frac {147-0}{100-0}}={\frac {147}{2}}=73.5\approx74}
$$
...
$$
{\displaystyle GreenChannelMidpoint={\frac {250+112}{2}}=181}
$$

$$
{\displaystyle BlueChannelMidpoint={\frac {154+219}{2}}=186.5\approx187}
$$

For the red and blue channels the value has to be rounded as it lies between valid incerements. Consequently the closest
...
The technique explained and then demonstrated above is rarely diectly useful while writing CSS, it is, however, very
...
colours `rgb(200, 125, 50)`, `rgb(50, 125, 200)`, `rgb(50, 200, 125)`, `rgb(200, 50, 125)`, `rgb(125, 200, 50)` and
`rgb(125, 50, 200)` at a glance we can tell that the following combinations will produce _muddy_ gradients:

 - `rgb(200, 125, 50)` &amp; `rgb(50, 125, 200)` &mdash; midpoint: rgb(125, 125, 125)
 - `rgb(50, 200, 125)` &amp; `rgb(200, 50, 125)` &mdash; midpoint: rgb(125, 125, 125)
 - `rgb(125, 200, 50)` &amp; `rgb(125, 50, 200)` &mdash; midpoint: rgb(125, 125, 125)

While the remaining combinations will produce colourful gradients:

 - `rgb(200, 125, 50)` &amp; `rgb(50, 200, 125)` &mdash; midpoint: rgb(125, 163, 88)
 - `rgb(200, 125, 50)` &amp; `rgb(200, 50, 125)` &mdash; midpoint: rgb(200, 88, 88)
 - `rgb(200, 125, 50)` &amp; `rgb(125, 200, 50)` &mdash; midpoint: rgb(163, 163, 50)
 - `rgb(200, 125, 50)` &amp; `rgb(125, 50, 200)` &mdash; midpoint: rgb(163, 88, 125)
 - `rgb(50, 125, 200)` &amp; `rgb(50, 200, 125)` &mdash; midpoint: rgb(50, 163, 163)
 - `rgb(50, 125, 200)` &amp; `rgb(200, 50, 125)` &mdash; midpoint: rgb(125, 88, 163)
 - `rgb(50, 125, 200)` &amp; `rgb(125, 200, 50)` &mdash; midpoint: rgb(88, 163, 125)
 - `rgb(50, 125, 200)` &amp; `rgb(125, 50, 200)` &mdash; midpoint: rgb(88, 88, 200)
 - `rgb(50, 200, 125)` &amp; `rgb(125, 200, 50)` &mdash; midpoint: rgb(88, 200, 88)
 - `rgb(50, 200, 125)` &amp; `rgb(125, 50, 200)` &mdash; midpoint: rgb(88, 125, 163)
 - `rgb(200, 50, 125)` &amp; `rgb(125, 200, 50)` &mdash; midpoint: rgb(163, 125, 88)
 - `rgb(200, 50, 125)` &amp; `rgb(125, 50, 200)` &mdash; midpoint: rgb(163, 50, 163)
...
```css caption=Dull_Gradient
/*----------------------------------------------------------------------------*\
 *
 * "Muddy" Gradients 
 * linear-gradient(rgb(200, 125, 50), rgb(50, 125, 200))
 * linear-gradient(rgb(50, 200, 125), rgb(200, 50, 125))
 * linear-gradient(rgb(125, 200, 50), rgb(125, 50, 200))
 *
 * Colourful Gradients
 * linear-gradient(rgb(200, 125, 50), rgb(50, 200, 125))
 * linear-gradient(rgb(200, 125, 50), rgb(200, 50, 125))
 * linear-gradient(rgb(200, 125, 50), rgb(125, 200, 50))
 * linear-gradient(rgb(200, 125, 50), rgb(125, 50, 200))
 * linear-gradient(rgb(50, 125, 200), rgb(50, 200, 125))
 * linear-gradient(rgb(50, 125, 200), rgb(200, 50, 125))
 * linear-gradient(rgb(50, 125, 200), rgb(125, 200, 50))
 * linear-gradient(rgb(50, 125, 200), rgb(125, 50, 200))
 * linear-gradient(rgb(50, 200, 125), rgb(125, 200, 50))
 * linear-gradient(rgb(50, 200, 125), rgb(125, 50, 200))
 * linear-gradient(rgb(200, 50, 125), rgb(125, 200, 50))
 * linear-gradient(rgb(200, 50, 125), rgb(125, 50, 200))
 *
\*----------------------------------------------------------------------------*/
background-image: linear-gradient(rgb(200, 125, 50), rgb(50, 125, 200));
```

Another potential use for this would be to shift the midpoint of a gradient. Going back to the `mediumspringgreen` and
`mediumpurple` example, shifting the midpoint down 10% is simply a case of adding the midpoint colour we calculated as a
`color-stop` and assigning it the position 60%.

```css caption=Shifted_Midpoint_Gradient
background-image: linear-gradient(mediumspringgreen, rgb(74, 181, 187) 60%, mediumpurple);
```

A similar effect can be achieved more concisely via another feature of CSS gradients.

## **Colour hints**
```css caption=Colour_Hinted_Gradient
background-image: linear-gradient(mediumspringgreen, 60%, mediumpurple);
```
